...
以下能力曾在 Maya 2025 真实角色绑定场景中手动运行验证；可重复 smoke 的覆盖范围另列在后文，不能把一次手动验证当成自动回归保证：
...
`tests/maya_smoke.py` 只验证当前打开场景中已保存 L/R 目标的回放与恢复：

- 60 滑栏权重为 `1.0`，59 滑栏权重在 `0.9-1.0`；
- 保存矩阵误差不超过 `1e-10`，镜像骨骼位置误差不超过 `1e-5`；
- BlendShape 连接、控制器矩阵和选择状态恢复，且整段操作可 undo；
- 目标数量随场景变化，不固定为 8 对。

创建、复制/修改、列表显示、双击回放、UI 宿主、失败恢复和镜像方向的具体手动证据仍以 Notes 的 `PLAN_adpose_local_angle_space_fix` 为准；当前仓库没有把这些入口全部自动化。
